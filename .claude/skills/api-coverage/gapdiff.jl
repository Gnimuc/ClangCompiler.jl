...
# a class as unwrapped when it was fully covered. These are the causes, handled here rather
# than left to the caller:
#
#   * Case. One mis-cased class segment hides a whole class: it once hid all 44 of
#     `clang::Value`'s methods, every one of which was bound. The compare is case-insensitive
#     so the diff does not depend on the shim spelling its own prefixes right.
