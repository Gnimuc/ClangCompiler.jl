...
1. **Case.** The shim lower-cases a few class segments (`clang_value_getBool`,
   `clang_sema_getTypeName`). A case-sensitive compare reported all 44 of `clang::Value`'s
   methods as missing when every one was bound.
