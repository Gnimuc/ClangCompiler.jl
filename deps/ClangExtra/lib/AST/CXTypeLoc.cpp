...
CXTypeLocClass clang_TypeLoc_getTypeLocClass(CXTypeLoc TL) {
  return static_cast<CXTypeLocClass>(
      static_cast<clang::TypeLoc *>(TL)->getTypeLocClass());
}

CXTypeLoc clang_TypeLoc_getUnqualifiedLoc(CXTypeLoc TL) {
  return new clang::TypeLoc( // NOLINT(*-owning-memory)
      static_cast<clang::TypeLoc *>(TL)->getUnqualifiedLoc());
}

CXTypeLoc clang_TypeLoc_IgnoreParens(CXTypeLoc TL) {
  return new clang::TypeLoc( // NOLINT(*-owning-memory)
      static_cast<clang::TypeLoc *>(TL)->IgnoreParens());
}

// QualifiedTypeLoc
CXTypeLoc clang_QualifiedTypeLoc_getUnqualifiedLoc(CXTypeLoc TL) {
  clang::QualifiedTypeLoc L =
      static_cast<clang::TypeLoc *>(TL)->castAs<clang::QualifiedTypeLoc>();
  return new clang::TypeLoc(L.getUnqualifiedLoc()); // NOLINT(*-owning-memory)
}

// TypeSpecTypeLoc
CXSourceLocation_ clang_TypeSpecTypeLoc_getNameLoc(CXTypeLoc TL) {
  return static_cast<clang::TypeLoc *>(TL)
...
CXTypeLoc clang_AttributedTypeLoc_getModifiedLoc(CXTypeLoc TL) {
  clang::AttributedTypeLoc L =
      static_cast<clang::TypeLoc *>(TL)->castAs<clang::AttributedTypeLoc>();
  return new clang::TypeLoc(L.getModifiedLoc()); // NOLINT(*-owning-memory)
}

CXAttr clang_AttributedTypeLoc_getAttr(CXTypeLoc TL) {
  return const_cast<clang::Attr *>(
      static_cast<clang::TypeLoc *>(TL)->castAs<clang::AttributedTypeLoc>().getAttr());
}
...
CXTypeLoc clang_AdjustedTypeLoc_getOriginalLoc(CXTypeLoc TL) {
  clang::AdjustedTypeLoc L =
      static_cast<clang::TypeLoc *>(TL)->castAs<clang::AdjustedTypeLoc>();
  return new clang::TypeLoc(L.getOriginalLoc()); // NOLINT(*-owning-memory)
}
...
unsigned clang_FunctionTypeLoc_getNumParams(CXTypeLoc TL) {
  return static_cast<clang::TypeLoc *>(TL)->castAs<clang::FunctionTypeLoc>().getNumParams();
}

CXParmVarDecl clang_FunctionTypeLoc_getParam(CXTypeLoc TL, unsigned i) {
  return static_cast<clang::TypeLoc *>(TL)->castAs<clang::FunctionTypeLoc>().getParam(i);
}
...
CXExpr clang_ArrayTypeLoc_getSizeExpr(CXTypeLoc TL) {
  return static_cast<clang::TypeLoc *>(TL)->castAs<clang::ArrayTypeLoc>().getSizeExpr();
}
...
#define TYPE(Class, Base)                                                                  \
  CXTypeLoc clang_TypeLoc_castTo##Class##TypeLoc(CXTypeLoc TL) {                           \
    clang::Class##TypeLoc L =                                                              \
        static_cast<clang::TypeLoc *>(TL)->getAs<clang::Class##TypeLoc>();                 \
    if (L.isNull())                                                                        \
      return nullptr;                                                                      \
    return new clang::TypeLoc(L); /* NOLINT(*-owning-memory) */                            \
  }
#define ABSTRACT_TYPE(Class, Base)
#include "clang-ex/AST/TypeNodes.inc"
...
CXTypeLoc clang_TypeLoc_castToQualifiedTypeLoc(CXTypeLoc TL) {
  clang::QualifiedTypeLoc L =
      static_cast<clang::TypeLoc *>(TL)->getAs<clang::QualifiedTypeLoc>();
  if (L.isNull())
    return nullptr;
  return new clang::TypeLoc(L); // NOLINT(*-owning-memory)
}
...
CXTypeLoc clang_TypeLoc_castToTypeSpecTypeLoc(CXTypeLoc TL) {
  clang::TypeSpecTypeLoc L =
      static_cast<clang::TypeLoc *>(TL)->getAs<clang::TypeSpecTypeLoc>();
  if (L.isNull())
    return nullptr;
  return new clang::TypeLoc(L); // NOLINT(*-owning-memory)
}
...
CXTypeLoc clang_TypeLoc_castToFunctionTypeLoc(CXTypeLoc TL) {
  clang::FunctionTypeLoc L =
      static_cast<clang::TypeLoc *>(TL)->getAs<clang::FunctionTypeLoc>();
  if (L.isNull())
    return nullptr;
  return new clang::TypeLoc(L); // NOLINT(*-owning-memory)
}
...
CXTypeLoc clang_TypeLoc_castToArrayTypeLoc(CXTypeLoc TL) {
  clang::ArrayTypeLoc L =
      static_cast<clang::TypeLoc *>(TL)->getAs<clang::ArrayTypeLoc>();
  if (L.isNull())
    return nullptr;
  return new clang::TypeLoc(L); // NOLINT(*-owning-memory)
}
