...
function resolve(ty::AbstractType)
    ...
    return T === nothing ? resolve(UnexposedType(ty)) : downcast(T, ty)
end

resolve(x::UnexposedType) = x

function resolve(ty::AbstractBuiltinType)
    is_void_ty(ty) && return downcast(VoidTy, ty)
    is_bool_ty(ty) && return downcast(BoolTy, ty)
    is_char_ty(ty) && return downcast(CharTy, ty)
    is_wchar_ty(ty) && return downcast(WCharTy, ty)
    is_widechar_ty(ty) && return downcast(WideCharTy, ty)
    is_signed_char_ty(ty) && return downcast(SignedCharTy, ty)
    is_short_ty(ty) && return downcast(ShortTy, ty)
    is_int_ty(ty) && return downcast(IntTy, ty)
    is_long_ty(ty) && return downcast(LongTy, ty)
    is_longlong_ty(ty) && return downcast(LongLongTy, ty)
    is_int128_ty(ty) && return downcast(Int128Ty, ty)
    is_unsigned_char_ty(ty) && return downcast(UnsignedCharTy, ty)
    is_unsigned_short_ty(ty) && return downcast(UnsignedShortTy, ty)
    is_unsigned_int_ty(ty) && return downcast(UnsignedIntTy, ty)
    is_unsigned_long_ty(ty) && return downcast(UnsignedLongTy, ty)
    is_unsigned_longlong_ty(ty) && return downcast(UnsignedLongLongTy, ty)
    is_unsigned_int128_ty(ty) && return downcast(UnsignedInt128Ty, ty)
    is_char8_ty(ty) && return downcast(Char8Ty, ty)
    is_char16_ty(ty) && return downcast(Char16Ty, ty)
    is_char32_ty(ty) && return downcast(Char32Ty, ty)
    is_float_ty(ty) && return downcast(FloatTy, ty)
    is_double_ty(ty) && return downcast(DoubleTy, ty)
    is_long_double_ty(ty) && return downcast(LongDoubleTy, ty)
    is_float16_ty(ty) && return downcast(Float16Ty, ty)
    is_half_ty(ty) && return downcast(HalfTy, ty)
    is_bfloat_ty(ty) && return downcast(BFloat16Ty, ty)
    is_float128_ty(ty) && return downcast(Float128Ty, ty)
    is_nullptr_ty(ty) && return downcast(NullPtrTy, ty)
    return resolve(UnexposedType(ty))
end

function resolve(ty::AbstractTagType)
    is_record_type(ty) && return downcast(RecordType, ty)
    is_enum_type(ty) && return downcast(EnumType, ty)
    return resolve(UnexposedType(ty))
end

function resolve(ty::AbstractFunctionType)
    is_function_no_proto_type(ty) && return downcast(FunctionNoProtoType, ty)
    is_function_proto_type(ty) && return downcast(FunctionProtoType, ty)
    return resolve(UnexposedType(ty))
end

function resolve(ty::AbstractReferenceType)
    is_lvalue_reference_type(ty) && return downcast(LValueReferenceType, ty)
    is_rvalue_reference_type(ty) && return downcast(RValueReferenceType, ty)
    return resolve(UnexposedType(ty))
end

function resolve(ty::AbstractArrayType)
    is_constant_array_type(ty) && return downcast(ConstantArrayType, ty)
    is_incomplete_array_type(ty) && return downcast(IncompleteArrayType, ty)
    is_variable_array_type(ty) && return downcast(VariableArrayType, ty)
    is_dependent_size_array_type(ty) && return downcast(DependentSizedArrayType, ty)
    return resolve(UnexposedType(ty))
end
