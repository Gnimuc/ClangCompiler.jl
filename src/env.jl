...
Executing under a pinned triple is a trap rather than a refusal, so pin with
`create_parser`, which has no JIT. Every backend the host's LLVM was built with is
registered, which is a short list that need not include the pinned architecture; outside it
an interpreter cannot be constructed at all. Inside it — a same-architecture pin across
operating systems, `x86_64-linux-gnu` on an x86_64 mac — code emits, links and runs using
another platform's headers and ABI, nothing raises, and tearing the JIT down can crash.
"""
