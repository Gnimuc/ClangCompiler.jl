...
# class, stamped from the ATTR_NODES table. The wrapped pointer is NULL when
# the attribute is not of that class (dyn_cast_or_null semantics).
for node in ATTR_NODES
    tsym = attr_carrier_name(node.name)
    pred = Symbol("clang_Attr_is", tsym)
    isname = Symbol("is", tsym)
    cast = Symbol("clang_Attr_castTo", tsym)
    @eval function $isname(x::AbstractAttr)
        @check_ptrs x
        return $pred(x)
    end
    @eval function $tsym(x::AbstractAttr)
        @check_ptrs x
        return $tsym($cast(x))
    end
end
