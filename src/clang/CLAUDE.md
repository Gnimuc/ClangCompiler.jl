...
Stmt hierarchy, stamped from the `STMT_NODES` table in `core/AST/StmtAbstract.jl`):
...
abstract type. Many carriers are stamped in a `for sym in [...] @eval` loop; append to the
loop rather than writing a standalone struct.
