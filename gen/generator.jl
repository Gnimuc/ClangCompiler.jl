    ...
    for (llvm_version, julia_version) in ((v"18.1.7", v"1.12"),)
        ...
        temp_prefix() do prefix
            ...
            let options = deepcopy(options)
                ...
                # each vendored node .inc defines hundreds of transient per-class
                # X-macros (WHILESTMT/EXPR…; FUNCTION/NAMESPACE…; NON_CANONICAL_TYPE…;
                # INHERITABLE_ATTR…); ignore them all, self-syncing with whatever
                # the pinned .inc files define
                for inc in ("StmtNodes.inc", "DeclNodes.inc", "TypeNodes.inc", "AttrList.inc")
